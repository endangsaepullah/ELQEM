...
def show():
    ...
    conn = get_connection()
    df = pd.read_sql(
        "SELECT id,username,full_name,role,email,is_active,created_at,last_login FROM users ORDER BY id",
        conn
    )
    ...
    with tab1:
        for _, row in df.iterrows():
            ...
            with st.expander(
                f"{'🔑' if row['role']=='admin' else '👁'} "
                f"{row['full_name'] or row['username']}  @{row['username']}"
            ):
    ...
    with tab2:
        with st.form("add_user"):
            ...
            if st.form_submit_button("➕ Tambah User", use_container_width=True, type="primary"):
                if uname and upw:
                    try:
                        conn.execute("""INSERT INTO users (username,password_hash,full_name,role,email,is_active)
                                        VALUES(?,?,?,?,?,1)""",
