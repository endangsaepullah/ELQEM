...
from utils.database import fetchone, execute, get_category, get_lifecycle_maturity
...
def seed_dummy_data():
    ...
    for u in [
        ...
        try:
            execute("INSERT INTO users (username,password_hash,full_name,role,email) VALUES (?,?,?,?,?)", u)
        except Exception:
            pass
    ...
    for i in range(1, 13):
        ...
        bid_row = fetchone("SELECT id FROM batch_production WHERE batch_number=?", (f"BATCH-KMN-2024-{i:03d}",))
    ...
    for i in range(6):
        ...
        execute("INSERT INTO iso9001_evaluation (eval_date,batch_number,process_documentation,process_control,internal_audit,corrective_action,continuous_improvement,average_score,category,evaluator) VALUES (?,?,?,?,?,?,?,?,?,?)",
        ...
        execute("INSERT INTO iatf16949_evaluation (eval_date,batch_number,risk_based_thinking,defect_prevention,supplier_quality,continuous_improvement,average_score,category,evaluator) VALUES (?,?,?,?,?,?,?,?,?)",
        ...
        execute("INSERT INTO engineering_lifecycle (eval_date,batch_number,design_control,change_control,verification_validation,integration_process,traceability,design_change_communication,average_score,maturity_level,evaluator) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        ...
        execute("INSERT INTO quality_consistency (eval_date,batch_number,quality_uniformity,low_defect_rate,inter_batch_stability,low_rework_rate,spec_conformance,average_score,category,evaluator) VALUES (?,?,?,?,?,?,?,?,?,?)",
    ...
    for row in [
        ...
        execute("INSERT INTO interview_data (interview_date,informant_name,position,work_unit,interview_result,key_insights,finding_category,interviewer) VALUES (?,?,?,?,?,?,?,?)", row)
